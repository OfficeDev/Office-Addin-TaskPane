...
hosts.forEach(async function (host) {
    await fs.unlinkFileAsync(`./../manifest.${host}.xml`);
    await fs.unlinkFileAsync(`./../src/taskpane/${host}.${extension}`);
});
...
//         const manifestContent = await readFileAsync(path.resolve(`${projectFolder}/manifest.${host}.xml`), 'utf8');
